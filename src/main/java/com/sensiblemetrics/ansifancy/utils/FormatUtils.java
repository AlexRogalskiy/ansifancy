...
import com.sensiblemetrics.ansifancy.enumeration.ConversionMarkerType;
...
public class FormatUtils {
    private static class Conversion {
        ...
        private final ConversionMarkerType cath;

        public Conversion(char c, int index) {
            this.index = index;
            this.cath = ConversionMarkerType.fromConversionChar(c);
        }
     ...
     * {@link ConversionMarkerType}s. Otherwise an {@link Error} is thrown.
     *
     * <p>TODO introduce more such functions, see RegexUtil for examples
     */
    public static String asFormat(String format, ConversionMarkerType... cc)
        throws IllegalFormatException {
        ConversionMarkerType[] fcc = formatParameterCategories(format);
        ...
        for (int i = 0; i < cc.length; i++) {
            if (cc[i] != fcc[i]) {
                throw new IllegalFormatConversionMarkerTypeException(cc[i], fcc[i]);
            }
        }
    ...
    /**
     * Returns a {@link ConversionMarkerType} for every conversion found in the format string.
     *
     * <p>Throws an exception if the format is not syntactically valid.
     */
    public static ConversionMarkerType[] formatParameterCategories(String format)
        throws IllegalFormatException {
        ...
        Map<Integer, ConversionMarkerType> conv = new HashMap<>();
        ...
        for (Conversion c : cs) {
            ...
            conv.put(
                last,
                ConversionMarkerType.intersect(
                    conv.containsKey(last) ? conv.get(last) : ConversionMarkerType.UNUSED,
                    c.category()));
        }

        ConversionMarkerType[] res = new ConversionMarkerType[maxindex + 1];
        for (int i = 0; i <= maxindex; ++i) {
            res[i] = conv.containsKey(i) ? conv.get(i) : ConversionMarkerType.UNUSED;
        }
        return res;
    }
    ...
    public static class IllegalFormatConversionMarkerTypeException
        extends IllegalFormatConversionException {
        private static final long serialVersionUID = 17000126L;

        private final ConversionMarkerType expected;
        private final ConversionMarkerType found;
        ...
        public IllegalFormatConversionMarkerTypeException(
            ConversionMarkerType expected, ConversionMarkerType found) {
            super(
        ...
        public ConversionMarkerType getExpected() {
            return expected;
        }

        public ConversionMarkerType getFound() {
            return found;
        }
